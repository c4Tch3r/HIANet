...
def dwt_init1d(x):
    ...
    return torch.cat((x_LL, x_HH), 1)


def iwt_init1d(x):
    r = 2
    in_batch, in_group, in_time = x.size()
    out_batch, out_group, out_time = in_batch, int(in_group / r), r * in_time
    x1 = x[:, 0:out_group, :]  # LL
    x2 = x[:, out_group:2*out_group, :]  # HH

    h = torch.zeros([out_batch, out_group, out_time]).float().cuda()
